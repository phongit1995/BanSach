...
module.exports = {
    validator: (req, errUpload, taskCurrent )=>{
        ...
        req.checkBody('author',util.format(notify.ERROR_AUTHOR)).notEmpty();
        req.checkBody('price',util.format(notify.ERROR_PRICE)).isNumeric();
        req.checkBody('coverprice',util.format(notify.ERROR_COVER_PRICE)).isNumeric();
        req.checkBody('pages',util.format(notify.ERROR_PAGES)).isNumeric();
